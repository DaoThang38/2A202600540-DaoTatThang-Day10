...
def supervisor_node(state: AgentState) -> AgentState:
    ...
    # Logic routing dựa trên worker_contracts.yaml
    ...
    # 1. Error không rõ (risk_high)
    if "err-" in task:
        risk_high = True
        route = "human_review"
        route_reason = "task chứa mã lỗi không rõ (ERR-xxx) VÀ không có context"
        
    # 2. Policy keywords (ưu tiên 2 nếu không phải ERR)
    elif any(kw in task for kw in policy_keywords):
        route = "policy_tool_worker"
        route_reason = "task chứa 'hoàn tiền', 'refund', 'flash sale', 'license', 'cấp quyền', 'access level'"
        needs_tool = True

    # 3. Retrieval keywords (ưu tiên 3)
    elif any(kw in task for kw in retrieval_keywords):
        route = "retrieval_worker"
        route_reason = "task chứa 'P1', 'SLA', 'ticket', 'escalation', 'sự cố'"
...
def build_graph():
    ...
    # Human Review always goes to Retrieval after approval
    workflow.add_edge("human_review", "retrieval_worker")
    
    # Workers go to Synthesis
...
def run_graph(task: str) -> AgentState:
    ...
    # LangGraph compile() returns an executable graph, we call .invoke()
